...
class Audio(Service):
    ...
    @Signal
    def stream_added(self, stream: AudioStream) -> None: ...
    @Signal
    def stream_removed(self, stream: AudioStream) -> None: ...
    ...
    def on_stream_added(self, _, stream_id: int):
        ...
        if (
            ...
            or not Audio.get_stream_type(stream, None)
            or not stream
        ):
            return  # No.
        ...
        self._stream_connectors[stream_id] = audio_stream.connect("changed", lambda *_: self.changed())

        logger.info(f"[Audio][{audio_stream.type.title()}] Adding stream {stream_id} with name {audio_stream.name}")

        self.do_notify_streams(stream)
        # fire the specific stream-added signal
        self.emit("stream-added", audio_stream)
        # also notify any listeners of the generic 'changed'
        return self.changed()
    ...
    def on_stream_removed(self, _, stream_id: int):
        ...
        logger.info(f"[Audio][{audio_stream.type.title()}] Removing stream {stream_id} with name {audio_stream.name}")
        self.do_notify_streams(audio_stream.stream)
        # fire the specific stream-removed signal
        self.emit("stream-removed", audio_stream)
        audio_stream.close()
        return self.changed()
